...
[![Version](https://img.shields.io/badge/version-v3.2.0-8b5cf6?style=flat-square&labelColor=1a1740)](https://github.com/272416939/pve-multi-user-panel)
...
| # | 功能 | 说明 |
|---|------|------|
| 15 | **虚拟机快照管理** | 创建/回滚/删除，配置全局限制（每 VM 上限、每日次数） |
| 16 | **虚拟机备份恢复** | PVE 停止模式 + zstd 压缩，异步轮询进度，恢复后自动磁盘对账审计 |
| 17 | **CDK 兑换码系统** | 批量生成、CSV 导出、指定用户分配、自动通知 |
| 18 | **DHCP 静态绑定** | VM/LXC 分配时自动创建，解绑销毁自动删除 |
| 19 | **端口转发管理** | ikuai 自动同步，随机端口、冲突检查 |
| 20 | **套餐库存管理** | 可选销售库存上限，售罄自动禁用，支持补货 |
| 21 | **任务状态徽标** | VM/LXC 列表显示「备份中/恢复中/切换中」状态徽标并锁定操作按钮，防误操作（v2.33.3 新增） |
...
| # | 功能 | 说明 |
|---|------|------|
| 25 | **现代化界面** | 深色科技风格 + 毛玻璃效果 + 霓虹光晕 |
| 26 | **响应式设计** | 适配各种屏幕尺寸 |
| 27 | **极客头像** | 基于用户名自动生成唯一的 SVG 电路板渐变头像 |
| 28 | **导航动态化** | 三页面导航统一通过 API 渲染，角色感知 |
| 29 | **套餐排序** | 自定义排序值，数字越小排越前 |
| 30 | **套餐自定义字段** | CPU 型号、带宽 (Mbps) 可配置 |
| 31 | **套餐备注 Markdown** | 备注支持 Markdown 语法渲染 |
| 32 | **界面模板（双模板）** | 赛博霓虹（霓虹赛博·紫青发光）+ SAAS 企业风模板，各含明暗配色；管理员在站点设置选择全站默认，用户可在个人设置自选覆盖（优先级：个人偏好 > 站点默认） |

### 💰 支付 & 交易（v2.0 新增）
| # | 功能 | 说明 |
|---|------|------|
| 32 | **在线充值** | 支付宝/微信支付，V1 MD5 + V2 RSA 签名双模式 |
| 33 | **余额续费** | 余额抵扣 VM/LXC 续费，支持月/年付 |
| 34 | **交易流水** | 完整支付记录，支付流水号（transaction_id）、类型、金额 |
| 35 | **支付配置** | 支付网关 PID/密钥/开关独立管理 |

### ⚡ 性能优化（v1.8.0-beta21~23）
| # | 功能 | 说明 |
|---|------|------|
| 36 | **WS 状态推送** | VM/LXC CPU/内存/网络 3s 实时推送，替代 HTTP 整表轮询 |
| 37 | **WS 监控图表** | 详情弹窗 4 组 Chart.js 图表通过 subscribe-detail 推送，消灭 3s HTTP 请求 |
| 38 | **进程内 TTL 缓存** | profile(60s) + unread-count(10s) + PVE 状态(5s) 缓存，减少 DB/PVE 调用 |
| 39 | **备份进度 WS 推送** | 备份/恢复完成后 pushToUser 实时通知，消灭 10s 前端轮询 |
| 40 | **PVE 状态复用** | GET /user/vms 和 /user/lxc 优先命中 pushStatus 缓存，PVE API 调用减半 |

### 🔄 自选系统功能（v2.33.0 新增）
| # | 功能 | 说明 |
|---|------|------|
| 41 | **自选操作系统** | 新购 VM 时必选 OS 模板（Windows/Linux 全系列），支持 PVE 模板 VMID 下拉 + 自动填充配置 |
| 42 | **在线系统切换** | 用户自助切换 VM 操作系统，基于 PVE move_disk API 统一 LVM/DIR 路径，数据盘不受影响 |
| 43 | **系统切换日志** | Admin 后台查看所有用户系统切换记录，支持筛选/分页/批量删除/详情查看 |
| 44 | **OS 模板管理** | 管理可切换系统模板（模板 VMID、磁盘格式、ostype、cloud-init 配置） |

### 📬 邮件通知增强（v2.33.0 新增）
| # | 功能 | 说明 |
|---|------|------|
| 45 | **扣款/退款邮件通知** | 购买/扩容/续费/销毁/开通失败退款等场景补齐邮件通知，含余额变动和订单号 |
| 46 | **用户邮件通知设置** | 用户可自定义接收哪些类型的邮件通知 |

### 💾 数据盘管理（v2.32.0 新增，持续迭代）
| # | 功能 | 说明 |
|---|------|------|
| 47 | **数据盘购买** | 用户自助购买 NVME/SATA/HDD/U2 数据盘，挂载到 VM，支持批量购买（上限 10） |
| 48 | **磁盘格式选择** | 购买/导入支持磁盘格式选择（qcow2/raw）与 DIR 存储类型，不支持扩容的格式自动警告提示（v2.32.2 新增） |
| 49 | **数据盘续费** | 独立计费周期（月/季/年），自动续费开关 |
| 50 | **数据盘扩容** | 随时扩容，按新容量价差收费，扩容部分独立计费 |
| 51 | **数据盘挂载/卸载** | 统一使用 SCSI 总线（支持热插拔，VM 运行中即可挂载/卸载），自动注入 QoS 限速参数；卸载后转移到中转 VM 托管，防止用户 VM 销毁连带删除卷文件（v2.33.1） |
| 52 | **存量磁盘导入** | 管理员手动扫描 PVE 中未管理的存量数据盘并导入台账；分配 VM 时自动导入（v2.33.0） |
| 53 | **Legacy 磁盘标识** | 导入的存量磁盘标记为 `legacy`，随 VM 计费，不独立续费 |
| 54 | **存储分组管理** | 存储分组创建/编辑/删除，支持拖拽排序＋动效，新建分组自动分配排序值，购买弹窗按排序展示（v2.32.1 新增） |
| 55 | **数据盘批量管理** | Admin 数据盘表格多选，一键批量迁移到目标存储分组（v2.32.1 新增） |
| 56 | **限速信息展示** | 用户购买弹窗展示带宽/IOPS 限速信息，Admin 规格卡片统一字段占位（v2.32.1 新增） |
| 57 | **数据盘管理（Admin）** | 管理后台查看/编辑/销毁所有用户数据盘，列表显示 PVE 路径＋分组规格徽标（v2.33.1）；备份恢复后自动磁盘对账审计（防白嫖） |
...
| 69 | **Redis 缓存** | 可选 Redis，速率限制/VNC ticket/提醒追踪持久化；**邮件队列**（BullMQ，v3.0.2） |
...
│   │   └── rate-limiter.js    # 统一速率限制（Redis/内存双模式）
	...
	│   │   └── tasks.js           # 定时任务
	│   ├── queue/
	│   │   └── email-queue.js     # 📬 邮件发送队列（BullMQ，异步化 + 降级同步，v3.0.2）
		│   ├── api/
...
| **硬盘管理** | 💾 购买/挂载/卸载/扩容/续费数据盘，支持按规格选购、磁盘格式选择、批量购买、限速信息展示 |
| **日志** | 操作日志/登录日志查看，筛选/搜索/导出/批量删除/清空（v2.33.8 新增） |
...
- **用途：** 作为创建磁盘卷时的 ID 占位符 + 扩容游离磁盘时的临时挂载点 + **卸载数据盘的托管位置**（v2.33.1 起，防止用户 VM 销毁时 PVE 连带删除 unused 卷文件）
...
4. **卸载托管：** 用户卸载的数据盘以 unused 卷形式托管到中转 VM，避免留在用户 VM 上被销毁时连带删除卷文件（v2.33.1）
...
管理员可在「硬盘设置 > 存储分组管理」中创建/编辑/删除存储分组（如 NVME-A、SATA-B），用于对磁盘类型进行分类。分组支持**拖拽排序＋动效**，新建分组自动分配排序值，用户购买弹窗按排序展示（v2.32.1）。
...
- 容量范围、单价、季付/年付折扣
- QoS 限速参数（MBPS/IOPS 读写限速及突发峰值），规格卡片统一字段占位，用户购买弹窗直接展示限速信息（v2.32.1）
- 磁盘格式选择（qcow2/raw），不支持扩容的格式给出警告提示（v2.32.2）
- 存储池关联（展示实时容量使用率）
...
管理员可在「硬盘设置 > 数据盘管理」中：
- **查看：** 所有用户的数据盘列表（ID/用户/名称/存储分组/规格/容量/状态/绑定VM/到期时间），列表显示 PVE 路径列＋分组规格名称徽标（v2.33.1）
- **编辑：** 修改磁盘名称、存储分组、规格
- **批量操作：** 表格多选，一键批量迁移选中磁盘到目标存储分组（v2.32.1）
- **销毁：** 3 天内全额退款，超过 3 天按剩余时间比例退款（不受 15 天限制，详见 [退款规则](#退款规则)）
- **导入存量磁盘：** 扫描 PVE 中未管理的存量数据盘，自动匹配规格并导入台账（标记为 `legacy`，随 VM 续费）；分配 VM 时也会自动导入存量数据盘（v2.33.0）

#### 用户硬盘管理

用户可在仪表盘「硬盘管理」中：
- **购买：** 选择规格/容量/磁盘格式（qcow2/raw）/周期，余额支付；弹窗展示带宽/IOPS 限速信息，不支持扩容的格式自动警告（v2.32.2）
- **挂载：** 将空闲磁盘挂载到自己的 VM（SCSI 总线热插拔，VM 运行中即可挂载），自动注入 QoS 限速
- **卸载：** 从 VM 卸载磁盘（卸载后转移到中转 VM 托管）
- **扩容：** 随时扩容，按价差收费
- **续费：** 月/季/年续费，支持自动续费
