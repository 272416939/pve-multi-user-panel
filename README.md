...
> **用户名:** `admin`  
> **密码:** 首次启动时随机生成 16 位强密码（基于 `crypto.randomBytes`）  
> ⚠️ 出于安全考虑，密码**不在控制台显示**。获取/自定义方式：
> - 在 `.env` 中设置 `DEFAULT_ADMIN_PASSWORD` 变量，启动时将使用该密码
> - 或启动后直接查看数据库 `users` 表（密码为加盐 SHA256，无法反推，可改可重置）
> 
> ⚠️ **首次登录后请立即修改密码！**
