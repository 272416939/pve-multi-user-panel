(function () {
    ...
    <div v-if="activeSection === 'i18n'">
        ...
        <div v-if="i18nPage.pendingLanguages.value.length > 0" class="i18n-pending-banner d-flex flex-wrap align-items-center gap-2 mb-3">
            ...
            <span class="banner-text">{{ t('admin.i18n.pendingLangs') }}：</span>
            <button v-for="l in i18nPage.pendingLanguages.value" :key="l.code" class="i18n-lang-chip" :class="{ 'i18n-lang-chip-active': l.code === i18nPage.selectedCode.value }" @click="i18nPage.selectLanguage(l.code)">{{ l.name }}（{{ tFormat('admin.i18n.pendingCount', l.pending) }}）</button>
        </div>
