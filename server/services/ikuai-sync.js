...
async function syncPortForwardsFromIkuai() {
    ...
    try {
        // 多节点化：遍历全部启用的爱快节点，逐节点拉取该设备的规则，
        // 与 DB 中 ikuai_node_id 归属该节点的规则对账导入/清理（旧全局逻辑作为循环体，幂等语义不变）
        const nodes = (await db.ikuaNodes.list()).filter(n => n.enabled && n.host);
        if (nodes.length === 0) {
            console.log('[ikuai] 启动同步: 无已启用的爱快节点，跳过');
            return;
        }
...
// 单个爱快节点的启动对账（原全局逻辑作为循环体）
async function syncPortForwardsForNode(n) {
    const ik = await getIkuaiClient(n.id);
    // 历史未落 ikuai_node_id 的存量规则视同默认爱快节点一并对账（避免重复导入/漏清理）
    let defIkNodeId = null;
    try { defIkNodeId = await db.ikuaNodes.getDefaultId(); } catch (_) {}
    ...
    let lanIps = [];
    if (ik.isConfigured()) {
        try { dhcpLeases = await ik.getDhcpLeases(); } catch (e) {}
        try { lanIps = await ik.getLanIps(); } catch (e) {}
    }
    ...
    for (let i = 0; i < allDevices.length; i += batchSize) {
        ...
        await Promise.all(batch.map(async (item) => {
            try {
                ...
                const pveApi = await getPveClient(item.device.pve_node_id);
                if (item.type === 'vm') {
