...
router.post('/lxc/:vmid/backups', authMiddleware, async (req, res) => {
    try {
        ...
        await auditAction(req, 'lxc.backup.create', '创建 LXC ' + vmid + ' 备份');
...
router.delete('/lxc/:vmid/backups/:id', authMiddleware, async (req, res) => {
    try {
        ...
        await auditAction(req, 'lxc.backup.delete', '删除 LXC ' + (backup.ct_id || req.params.vmid) + ' 备份', { resourceType: 'backup', resourceId: backup.ct_id || req.params.vmid });
...
router.post('/lxc/:vmid/backups/:id/restore', authMiddleware, async (req, res) => {
    try {
        ...
        await auditAction(req, 'lxc.backup.restore', '恢复 LXC ' + vmid + ' 备份');
...
router.post('/backups/batch-delete', authMiddleware, async (req, res) => {
    try {
        ...
        for (const id of ids) {
            ...
            // 操作审计：批量删除逐条记录明细
            if (backup.type === 'lxc') {
                await auditAction(req, 'lxc.backup.delete', '删除 LXC ' + (backup.ct_id || '') + ' 备份', { resourceType: 'backup', resourceId: backup.ct_id || '' });
            } else {
                await auditAction(req, 'vm.backup.delete', '删除 VM ' + (backup.vm_id || '') + ' 备份', { resourceType: 'backup', resourceId: backup.vm_id || '' });
            }
            if (backup.filename && backup.status === 'completed') {
